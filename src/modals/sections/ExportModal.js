...
// import { printResume } from '../../../functions';
  ...
  const handleDownload = async (isSinglePDF) => {
    ...
    try {
      const result = await fetch(
        `http://localhost:5000/screenshot?id=${state.id}&type=${
          isSinglePDF ? 'single' : 'multi'
        }`,
      );
      const data = await result.json();
      const blob = b64toBlob(data.data, 'application/pdf');
