...
export const formatDateRange = (
  {startDate, endDate, language = 'en', fallback = true},
  t,
) => {
  const start = startDate
    ? `${dayjs(startDate).locale(language.substr(0, 2)).format('MMMM YYYY')}`
    : '';
...
export const errorExtractor = (text) => {
  const error = JSON.parse(text?.message);
  let message = 'Languages.Oops';
